...
const mice = [
  ...
  { id: 19, name: "Sony INZONE M10", brand: "Sony", weight: 48, sensor: "Custom Sony", dpi: 30000, pollingRate: 8000, shape: "Symmetrical", connectivity: "Wireless", price: 149, switches: "Optical", proUsage: 0, rating: 9.1, image: "gamepad", color: "#3b82f6", batteryLife: 90, clicks: "100M", cable: "USB-C", releaseYear: 2025 },
...
export default function EsportsMice() {
  ...
  useEffect(() => {
    const handlePopState = () => {
      const hash = window.location.hash.replace('#', '');
      if (['overview','mice','rankings','sensors','players','games','brands','trends','compare'].includes(hash)) {
        setActiveTab(hash);
      }
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl p-6" style={{ background: `${brandCol}08`, border: `1px solid ${brandCol}20` }}>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      {MOUSE_IMAGE_URLS[p.mouse] ? <img src={MOUSE_IMAGE_URLS[p.mouse]} alt={p.mouse} className="h-14 w-14 object-contain" style={{ filter: "drop-shadow(0 4px 8px rgba(0,0,0,0.4))" }} /> : <span className="inline-block">{icon(currentMouseData?.image || "mouse", 40)}</span>}
                      <div>
